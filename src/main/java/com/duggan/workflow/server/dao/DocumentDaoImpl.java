...
public class DocumentDaoImpl extends BaseDaoImpl {
	...
	public List<DocumentModel> getAllDocuments(DocStatus... status) {
		...
		return getEntityManager()
				.createQuery(
						"FROM DocumentModel d where status in (:status) and createdBy=:createdBy and isActive=:isActive")
	...
	public DocumentModel getById(Long id) {
		List lst = getEntityManager()
				.createQuery("FROM DocumentModel d where id= :id")
	...
	public Long getProcessInstanceIdByDocumentId(Long documentId) {
		Object processInstanceId = getEntityManager()
				.createQuery(
	...
	public Long getProcessInstanceIdByDocRefId(String docRefId) {
		Object processInstanceId = getSingleResultOrNull(getEntityManager()
				.createQuery(
						"select n.processInstanceId FROM DocumentModel n"
								+ " where n.refId=:refId").setParameter(
						"refId", docRefId));
	...
	public List<DocumentModel> search(String userId, SearchFilter filter) {
		...
		Query hquery = getEntityManager().createQuery(query.toString());
...
//				expirationTime, processId, processSessionId, subTaskStrategy, parentId);
	...
	public ADDocType getDocumentTypeById(Long id) {
		...
		ADDocType docType = (ADDocType) getEntityManager()
	...
	public ADDocType getDocumentTypeByName(String docTypeName) {
		...
		docType = (ADDocType) getEntityManager()
	...
	public ADDocType getDocumentTypeByDocumentId(Long documentId) {
		...
		// ADDocType type =
		// (ADDocType)getEntityManager().createQuery("select d.type FROM DocumentModel d where d.id=:documentId")
		// .setParameter("documentId", documentId).getSingleResult();
		...
		Query query = getEntityManager()
				.createNativeQuery(sql, ADDocType.class).setParameter(1,
						documentId);
	...
	public String getDocumentTypeDisplayNameByDocumentId(Long documentId) {
		...
		Query query = getEntityManager().createNativeQuery(sql).setParameter(1,
				documentId);
	...
	public List<ADDocType> getDocumentTypes(String userId) {
		...
		List<ADDocType> docTypes = getResultList(getEntityManager()
				.createQuery(
						"SELECT distinct "
								+ "new com.duggan.workflow.server.dao.model.ADDocType("
								+ "t.id," + "t.name," + "t.display,"
								+ "t.className," + "t.category) " + "FROM "
								+ "ADDocType t "
	...
	public Long getFormId(Long documentTypeId) {
		...
		try {
			BigInteger bint = (BigInteger) getEntityManager()
	...
	public DetailModel getDetailById(Long detailId) {
		return (DetailModel) getEntityManager()
				.createQuery("FROM DetailModel d where id=:id")
				.setParameter("id", detailId).getSingleResult();
	}

	public TaskDelegation getTaskDelegationByTaskId(Long taskId) {
		try {
			return (TaskDelegation) getEntityManager()
	...
	public String getDocumentSubject(Long documentId) {
		...
		String value = (String) getEntityManager().createNativeQuery(sql)
	...
	public DocumentModel getDocumentByIdAndUser(String refId, String userId) {
		...
		Query query = getEntityManager().createQuery(jpql)
				.setParameter("refId", refId).setParameter("userId", userId);
