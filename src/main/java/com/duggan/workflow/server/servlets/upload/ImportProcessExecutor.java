...
public class ImportProcessExecutor extends FileExecutor {

	Logger log = Logger.getLogger(ImportProcessExecutor.class);
		...
		for (FileItem item : sessionFiles) {
			if (false == item.isFormField()) {
				try {
					...
					ProcessDaoHelper.importProcessAsStream(name,size,item.getInputStream());
