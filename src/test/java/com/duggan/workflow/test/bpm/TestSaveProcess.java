...
//		Assert.assertTrue(models.size()==1);
	...
	@Ignore
	public void update(){
		ProcessDef def = ProcessDaoHelper.getProcessDef(7L);
		//def.setDocTypes(Arrays.asList(DocType.CONTRACT));
		
		ProcessDaoHelper.save(def);
		
		def = ProcessDaoHelper.getProcessDef(7L);
		Assert.assertTrue(def.getDocTypes().size()==1);
	}
	
	@Ignore
	public void delete(){
		ProcessDaoHelper.delete(14L);
	}
...
//		/ProcessDocModel model = dao.getProcessDoc(DocType.LPO);
	...
	public void create(){
		...
		processDef = ProcessDaoHelper.save(processDef);
...
//		Assert.assertTrue(docTypes.size()>0);
		...
		List<ProcessDef> processes = ProcessDaoHelper.getAllProcesses("",true);
