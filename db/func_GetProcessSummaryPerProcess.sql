drop function if exists fun_GetProcessSummaryPerProcess(varchar, DATE,DATE);

CREATE FUNCTION fun_GetProcessSummaryPerProcess(in p_processId varchar(100),in p_start_date DATE, in p_end_date DATE) 
returns TABLE (
...
BEGIN
	RETURN QUERY 	
		...
		select count(*) total, i.status,i.processId from processinstancelog i 
		where i.start_date >= p_start_date and i.start_date<=COALESCE(p_end_date, CURRENT_DATE) 
		and i.processId=p_processId 
		group by i.processId,i.status) as q1) as q2 group by q2.processId) as base1
		inner join 
