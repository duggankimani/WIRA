drop function if exists fun_GetTaskSummary(DATE,DATE);

CREATE FUNCTION fun_GetTaskSummary(in p_start_date DATE, in p_end_date DATE) 
returns TABLE (
	refId varchar(30),
	ownerid varchar(100),
	fullName text,
...
BEGIN
	RETURN QUERY 	
		...
		where l.status=0 and start_date >= cast(p_start_date as DATE) and start_date<=COALESCE(cast(p_end_date as  DATE), CURRENT_DATE)
		) as base1 
		...
		inner join processinstancelog l 
		on (l.processinstanceid=t.processinstanceid) 
		where l.status=0 and start_date >= cast(p_start_date as DATE) and start_date<=COALESCE(cast(p_end_date as  DATE), CURRENT_DATE)) as base1 
